    ...
    for file_name in file_list:
        ...
        if "OUTCAR" in file_name:
            ...
            for line in outcar_file.readlines() :
                ...
                if "VRHFIN" in line :
                    VRHFIN_atom_name = line[11:13]
                    if ":" in VRHFIN_atom_name:
                        VRHFIN_atom_name = VRHFIN_atom_name.replace(":"," ")
                    atom_type.append(VRHFIN_atom_name)
                    print(VRHFIN_atom_name,end=" ")
                ...
                if "energy  without entropy" in line :
                    ...
                    #print("energy  without entropy=  " , line[30:45])
                    #force_convergence.append( [float (line[46:56]) ,float (line[59:70]) ,float (line[73:84])]) 
                ...
                #outcar_info["force_convergence"] = force_convergence
...
#print(outcar_dir)
    ...
    for key in outcar_dir:
        ...
        #print (outcar_dir[key]["atoms_info"] ,  key.rjust(20)  ,"energy_without_entropy ="+str(outcar_dir[key]["energy_without_entropy"][-1]).rjust(20)   )
    
        outcar_dir_key.append(key)
    outcar_dir_key.sort()
    for key in outcar_dir_key:
    
        print (outcar_dir[key]["atoms_info"] ,  key.rjust(20)  ,"energy_without_entropy ="+str(outcar_dir[key]["energy_without_entropy"][-1]).rjust(20) ,"  ",outcar_dir[key]["general_timing"]  )
...
#---------
    ...
    for j,k ,m,n ,s,t in where  :
        ...
        fileXYZ.writelines("REMARK   {}   \n".format(cont_lines))
        cont_lines+=1


        
        line_numbers=[]
        for line in get_file[m:n]:
            line=line+" "
            numbers = []
            number = ""
            #print(line)
        ...
        b=[" ","  "]
        for line in get_file[s:t]:
            angle = get_angle(line_numbers[0][:3] ,line_numbers[1][:3],line_numbers[2][:3]  )
            #print(angle)
            fileXYZ.writelines( "CRYST1   "+line[4:10]+"  "+line[16:23]+"  " +line[29:36]  +(b[0] if angle[0]>=100.0 else b[1] ) +  str("%.2f" %angle[0] )+ (b[0] if angle[1]>=100.0 else b[1] )+str("%.2f" %angle[1]  )+(b[0] if angle[2]>=100.0 else b[1] )+str("%.2f" %angle[2] )+ " P1"+"\n" )
...
StartFinder()                #you must first have this line to initialization

for key in  outcar_dir:
    OUT2PDB(key)
    PlotConvergenceLine(key)
